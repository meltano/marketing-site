...
import type { BlogPostPageData } from "@/lib/wp/site-pages";
...
function extractTextFromElement(el: Element): string {
  return (el.children || [])
    .map((child: ChildNode) => {
      ...
      if (child.type === "tag") return extractTextFromElement(child as Element);
      return "";
    })
    .join("");
}
...
function buildParserOptions() {
  return {
    replace: (domNode: DOMNode) => {
      ...
      if (!cls.includes("wp-block-preformatted")) return;
      return <HighlightedCode code={extractTextFromElement(domNode)} />;
    },
  };
}
...
export default function BlogPageView({ data }: BlogPageViewProps) {
  const { post } = data;
  const featuredImage = {
          ...
          <div className="blog-metadata">
            ...
            <div className="blog-metadata-author">
              <img
                ...
                src={wpAuthor(post.author)?.node?.avatar?.url || ""}
                alt={wpAuthor(post.author)?.node?.name || ""}
              />
                ...
                <div className="container-small">
                  <div id="toc">
                    <div className="toc-header">
                      ...
                      <button
                        type="button"
                        className={`toc-toggle ${isOpen ? "toc-opened" : "toc-closed"}`}
                        onClick={toggleTOC}
                      >
